...
if (params.txid!="zero"){
    ...
    */
    ...
    - Last adjusted: 24/08/2020  (minor errors)
    - Faced problems:
...
/*
    //Added extra step but removed again when problem was fixed: 
    preparation of scheme, because of compatibility issues with chewBBACA v2.1.0 and v2.5.0
    process prepscheme {
        publishDir "$cgMLSTx/", mode:'copy', overwrite: true
...
*/
    ...
    process allelcall {
        ...
        --cpu $params.cpu --ptf $trainingx/txid${txid}-${suffix}.trn --fc
        """
        //added --fc to force continue: if question is asked: yes (due to version update)
    }
    ...
    process gquality {
        ...
        script:
        """
        chewBBACA.py TestGenomeQuality -i $cgMLSTx/MLST-${txid}-${suffix}/wgMLST/alleleCallMatrix_cg.tsv \
        -n 13 -t 200 -s 5
        """
    }
    ...
    // removed copying the "_short.fasta_bsr.txt" files after the chewBBACA update to v2.4.6
    process cgMLSTloci{
        echo true
    ...
    process feedback{
        ...
        print ("WG-MLST SCHEME: scheme-${txid}-${suffix}")
        allcall= "$reportWG"
        txt2 = open (allcall, 'r')
        counter2 = 0
        for line2 in txt2:
        ...
        print ("CG-MLST SCHEME: scheme-${txid}-${suffix}-cgMLST")
        MLST= "$counts"
        txt3 = open (MLST, 'r')
        counter3 = 0
        for line3 in txt3:
