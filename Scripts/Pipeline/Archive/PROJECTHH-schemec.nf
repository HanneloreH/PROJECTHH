...
def helpMessage() {
    ...
    example1:   nextflow run PROJECTHH-scheme.nf --txid "817"
    ...
    --exist     give path to existing cgMLST scheme that requires modification for chewBBACA analysis
...
//only execute the next steps if TXID was defined by user AND if the scheme doesn't already exists
if (params.txid!="zero" && params.exist!="zero"){
    ...
    */
    ...
    //Split txid into 2 for processes: training and following and make sure processes aren't started too early
    //DELETE????????????????
...
'''
    //create BSR files (is not done automatically??) by adjusting existing file
    //is this file needed??
    process wgMLST {
...
'''
    ...
    // REMARK: TIME CONSUMING (>> depending on number of genomes)
    // REMARK: IN PIPELINE GOES WRONG: only part of the loci are used???? the ones with short fasta bsr!
    process allelcall {
        publishDir "$cgMLSTx/MLST-${txid}-${suffix}/wgMLST/", mode:'copy', overwrite: true
            ...
            for line in cfil:
                ...
                for item in fileList:
                    if(line == item):
                        ...
                        #lineB= line2 + "_short.fasta_bsr.txt"  #not generated with pipeline!
    ...
    process feedback{
        ...
        //give number of loci wgMLST : logging information
        ...
        for line1 in txt1:
            ...
            counter1+=1
        txt1.close()
        print("out of ${suffix} requested")
        print(" ")

        print ("WG-MLST ANALYSIS")
        allcall= "$reportWG"
        txt2 = open (allcall, 'r')
        counter2 = 0
        for line2 in txt2:
    ...
    // =============================  Finished message ====================================
    workflow.onComplete {
        log.info ( workflow.success ? "\ncgMLST SCHEME CREATED SUCCESSFULLY!: \nfinished script for txid ${TXID} - reference count: ${params.count}\n\n" : "Oops something went wrong, please check your input again!" )
    }
